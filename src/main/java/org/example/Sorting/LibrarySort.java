...
//    private final int arrayLength = e * (primaryLength + 1) - 1;//Формула для расчета количества элементов во вспомогательном массиве.
    ...
    public void sortArray(boolean localRebalance) {
        int[] primaryArray = generationDataRepository.getCoffeeListForSorted(); //Основной массив
        ...
        while (i < primaryLength) {
            ...
            if (supportingArray[pos] != 0) {
                ...
                if (localRebalance) {
                    ...
                    rebalanceLocal(primaryArray[i++], pos, arrayLength);
                    continue;
                } else {
        ...
        for (i = 0; i <= arrayLength - 1; i++) {
            if (supportingArray[i] != 0) {
                primaryArray[pos++] = supportingArray[i];
            }
        }
        System.out.println("Количество перестановок " + counter);
        System.out.println("РЕЗУЛЬТАТ, Основной массив после сортировки " + Arrays.toString(primaryArray));
        writingToTheRepository(primaryArray);
    }
    ...
    private int sortPos(int number, int count, int arrayLength) {
    ...
    private int posNearby(int start, int arrayLength) {
    ...
    private int binarySearch(int search, int start, int finish, int arrayLength) {
    ...
    private void rebalanceLocal(int insert, int pos, int arrayLength) {
    ...
    private void rebalanceTotal(int count, int arrayLength) {
        ...
        while (i >= 0) {
            if (supportingArray[i] != 0) {//Не пустой элемент
                int pos = sortPos(libraryNumber, count, arrayLength); //Его надо перенести сюда
    ...
    private void removeLeft(int i, int pos, int count, int arrayLength, int libraryNumber, int libraryLeftPos) {
        ...
        int left = 0;
        int leftPos = 0;//Пока ближайший сосед слева не найден
